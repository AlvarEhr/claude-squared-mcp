...
- **Mixed-version MCP processes are unsupported — restart every session after installing.** This protects data, not just behavior: a pre-0.14 process still drops registry entries it can't parse when it next writes the file (a quarantined entry survives only while every writer knows to preserve it); it can only be stopped through the compatibility marker (per-pair, so it stops whatever that process is executing on the pair); and a pre-0.13 process can't read the neutral vocabulary at all.
